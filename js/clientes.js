window.onload = function(){
    ...
    document.getElementById("plantillacontrol").onclick = function() {
        console.log("Vamos a insertar modificaciones o eliminar elementos");
            ...
            .then(data => {
                ...
                data.forEach(item => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${item.nombre}</td>
                        <td>
                            <button class="button delete" onclick="eliminarDato(${item.id})">Eliminar</button>
                            <button class="button modify" onclick="modificarDato(${item.id})">Modificar</button>
                        </td>
                    `;
    ...
    function eliminarDato(id) {
        if (confirm('¿Estás seguro de que deseas eliminar este dato?')) {
            fetch(`../API/eliminar_cliente.php?id=${id}`, {
                method: 'GET'
            })
    ...
    function modificarDato(id) {
        ...
        if (nuevoNombre) {
            fetch(`modificar.php?id=${id}&nombre=${nuevoNombre}`, {
                method: 'GET'
            })
            .then(response => response.text())
            .then(data => {
